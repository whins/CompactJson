namespace CompactJson
{
    ...
    public class JsonReader : IDisposable
    {
        ...
        {
            return ReadBooleanValue(JsonTokens.StringDelimiter);
        }
        public virtual string ReadString()
        {
            bool isNull = AssertAndConsume(JsonTokens.StringDelimiter);
            if (isNull)
            {
                SkipWhiteSpaces();
                return null;
            ...
            //return str.ToLower() == "null" ? "" : str;
            return str.ToLower() == "null" ? null : str;
        ...
        protected internal virtual bool ReadBooleanValue(char character)
        {
            char c = Read();
            if (c != character)
            {
        ...
        protected internal virtual bool AssertAndConsume(char character)
        {
            char c = Read();
            if (c != character)
            {
                if ('n' == c && 'u' == Read() && 'l' == Read() && 'l' == Read())
                {
                    return true;
                }
                throw new JsonException(string.Format("Expected character '{0}', but got: '{1}'", character, c));
        ...
        protected internal bool AssertNextIsDelimiterOrSeparator(char endDelimiter)
        {
            ...
            if (delimiter == JsonTokens.ElementSeparator)
            {
                return false;
            }
